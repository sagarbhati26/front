...
export default function AddTripModal({
  ...
  useEffect(() => {
    if (editingTrip) {
      setForm({
        ...
        to: editingTrip.to || "",
        dateTime: editingTrip.dateTime || editingTrip.rawDateTime || "",
        price: editingTrip.price?.toString() || "",
        totalSeats: editingTrip.totalSeats?.toString() || "",
  ...
  const validateTimes = () => {
    if (!form.departTime || !form.arriveTime) return true;

    const dep = new Date(`2000-01-01T${form.departTime}`);
    const arr = new Date(`2000-01-01T${form.arriveTime}`);
  ...
  const handleSubmit = async (e: any) => {
    ...
    if (!validateTimes()) {
      setErr("Arrival time cannot be earlier than departure time.");
      setLoading(false);
      return;
    }
